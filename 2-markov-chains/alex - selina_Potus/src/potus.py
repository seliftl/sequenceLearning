...
def tokenize(input, punctuation_patterns= DEFAULT_PUNCTUATIONS, split_characters = r'\s|\t|\n|\r', delimiter_token='<SPLIT>'):
    ...
    tk = TweetTokenizer()  
    list_of_token_strings = tk.tokenize(input_to_tokenize)

    # To lower case
    input_to_tokenize = input_to_tokenize.lower()
    list_of_token_strings = [x.lower() for x in list_of_token_strings]
...
def train_model(n, training_data, vocab):
    train_data, padded_sents = padded_everygram_pipeline(n, training_data)
    lm = Laplace(n)
    lm.fit(train_data, padded_sents)
    return lm

def get_models_for_president(train_data, vocab):
    models = {}
    for i in range(1,6):
        models[i] = train_model(i, train_data, vocab)
        print('Model for ', i, '-grams trained')
    return models

# %%
trump_models = get_models_for_president(trump_data['train'], trump_vocab)
obama_models = get_models_for_president(obama_data['train'], obama_vocab)
biden_models = get_models_for_president(biden_data['train'], biden_vocab)
